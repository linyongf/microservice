...
public class SysUserServiceImpl implements ISysUserService
{
    ...
    public User getUserByUsername(String username) {
        Wrapper<User> wrapper = Wrappers.lambdaQuery(User.class).eq(User::getUsername, username);
        User user = this.baseMapper.selectOne(wrapper);
        List<Map<String, String>> permissions = selectPermissionByUsername(username);
        if(CollectionUtils.isEmpty(permissions)){
            return user;
        }
        Collection<User.UserAuthority> authorities = new ArrayList<>();
        for (Map<String, String> permission : permissions) {
            ...
            User.UserAuthority oAuth2UserAuthority = new User.UserAuthority(tempMap);
